...
struct bloom *bloom_new(int size) {
    struct bloom *bf = malloc(sizeof(struct bloom));
    bf->size = size;
    bf->bit_vector = bitset_new(size);
    return bf;
}
...
void bloom_union(struct bloom *dest, struct bloom *src1, struct bloom *src2) {
    if (bitset_size(dest->bit_vector) == bitset_size(src1->bit_vector) &&
        bitset_size(dest->bit_vector) == bitset_size(src2->bit_vector)) {
        for (int i = 0; i < dest->bit_vector->size_in_words; i++) {
